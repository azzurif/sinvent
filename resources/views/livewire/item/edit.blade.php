<div>
  <form class="w-full" wire:submit="save">
    ...
    <section
      class="dark:bg-cold-black mb-4 flex w-full items-center gap-4 rounded-md border border-dark-gray bg-light-gray p-2">
      <!-- Category -->
      <div class="grow">
        ...
        <select
          ...
          @foreach ($categories as $category)
            <option value="{{ $category->id }}" @selected($category->id === $item->category_id)>{{ $category->category }}</option>
          @endforeach
        </select>
    ...
    <section
      class="dark:bg-cold-black flex w-full items-start gap-4 rounded-md border border-dark-gray bg-light-gray p-2">
      <!-- Specification -->
      <div class="grow">
